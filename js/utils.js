...
const ThemeManager = {
  ...
  set(theme)  {
    ...
    if (ic) ic.innerHTML = theme === 'dark' ? '<i class="bi bi-sun-fill"></i>' : '<i class="bi bi-moon-fill"></i>';
  },
  load(saved) {
    const t = saved || localStorage.getItem('rms_theme') || 'dark';
    document.documentElement.setAttribute('data-theme', t);
    const ic = document.getElementById('themeIcon');
    if (ic) ic.innerHTML = t === 'dark' ? '<i class="bi bi-sun-fill"></i>' : '<i class="bi bi-moon-fill"></i>';
  }
};
...
const Toast = {
  ...
  show(msg, type = 'info', dur = 3400) {
    ...
    el.innerHTML =
      `<span style="font-size:17px;flex-shrink:0;display:flex;align-items:center">${icons[type]||'•'}</span>` +
      `<span style="flex:1;font-size:13px">${msg}</span>` +
      `<span onclick="this.parentElement.remove()" style="cursor:pointer;color:var(--text-3);font-size:16px;flex-shrink:0"><i class="bi bi-x"></i></span>`;
...
function buildSidebar(uid) {
  ...
  sb.innerHTML = `
    <div class="sidebar-brand">
      <div class="brand-logo">${logo ? `<img src="${logo}" alt="logo">` : '<i class="bi bi-shop"></i>'}</div>
    ...
    <div class="nav-section">
      ...
      <a class="nav-item" href="dashboard.html"><span class="nav-icon"><i class="bi bi-speedometer2"></i></span><span>Dashboard</span></a>
      <a class="nav-item" href="tables.html">  <span class="nav-icon"><i class="bi bi-grid-1x2-fill"></i></span><span>Tables</span></a>
      <a class="nav-item" href="orders.html">  <span class="nav-icon"><i class="bi bi-cart3"></i></span><span>Orders</span>
        <span class="nav-badge" id="pendingBadge" style="display:none">0</span>
      </a>
      <a class="nav-item" href="kitchen.html"> <span class="nav-icon"><i class="bi bi-person-badge-fill"></i></span><span>Kitchen</span></a>
    </div>

    <div class="nav-section">
      <div class="nav-label">Management</div>
      <a class="nav-item" href="menu.html">    <span class="nav-icon"><i class="bi bi-pizza"></i></span><span>Menu</span></a>
      <a class="nav-item" href="billing.html"> <span class="nav-icon"><i class="bi bi-credit-card"></i></span><span>Billing</span></a>
      <a class="nav-item" href="history.html"> <span class="nav-icon"><i class="bi bi-journal-text"></i></span><span>History</span></a>
    </div>

    <div class="nav-section">
      <div class="nav-label">Account</div>
      <a class="nav-item" href="profile.html"> <span class="nav-icon"><i class="bi bi-person-fill"></i></span><span>Profile</span></a>
    </div>
...
function buildTopbar(title, sub) {
  ...
  if (!tb) return;
  tb.innerHTML = `
    <button class="icon-btn" onclick="toggleSidebar()" style="flex-shrink:0"><i class="bi bi-list"></i></button>
    ...
    <button class="icon-btn" onclick="ThemeManager.toggle()" title="Toggle theme">
      <span id="themeIcon">${ThemeManager.get() === 'dark' ? '<i class="bi bi-sun-fill"></i>' : '<i class="bi bi-moon-fill"></i>'}</span>
    </button>
    <button class="icon-btn" onclick="auth.signOut().then(()=>location.href='index.html')" title="Sign out"><i class="bi bi-door-open-fill"></i></button>`;
}
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
      <div class="modal-content">
        <div class="modal-body" style="padding:26px;text-align:center">
          <div style="font-size:34px;margin-bottom:12px"><i class="bi bi-exclamation-triangle-fill text-warning"></i></div>
...
function loadMongoImage(el, imageId, fallbackEmoji) {
  ImageService.loadInto(imageId, el, fallbackEmoji || '<i class="bi bi-bootstrap-reboot"></i>');
}

/* ============================================================
   © 2025 Shanudha Tirosh — https://github.com/ShanudhaTirosh
   ============================================================ */